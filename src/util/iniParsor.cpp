...
SINGLETON_DEFINE(IniParsor)

void GarvinEngine::Util::IniParsor::listSections(std::ifstream& in, std::vector<std::string> & vecSection)
{
	if (in.bad()) return;
...
void GarvinEngine::Util::IniParsor::listKV(std::ifstream& in, const std::string& section, DataTable* table)
{
...
void GarvinEngine::Util::IniParsor::_remSectionFormat(std::string& section)
{
	section = section.substr(1, section.length() - 2);
}

void GarvinEngine::Util::IniParsor::_addSectionFormat(std::string& section)
{
	section = "[" + section + "]";
}
